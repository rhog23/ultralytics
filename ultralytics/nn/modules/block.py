...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad, SimAM, CBAM
...
class BottleneckCBAM(nn.Module):
    ...
    def forward(self, x):
        """Applies the YOLO FPN to input data."""
        return (
            x + self.cbam(self.cv2(self.cv1(x)))
            if self.add
            else self.cbam(self.cv2(self.cv1(x)))
        )
...
class C2fCBAM(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(
            Bot(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0)
            for _ in range(n)
        )
